...
//              REPO
// ════════════════════════════════════════════
case 'repo': case 'github': case 'sc': {
    await react('💻');

    const sc = settings.fonts.smallCaps;
...
> SYSTEM: BOT_REPOSITORY_ACCESS
> CLASS: CORE_EMPIRE_LINKS
> OWNER: Mr Elephant
> RUNTIME: Node.js + Baileys MD
...
┃
┃ 5. ⚡ ICON-X MD BOT
┃ └─ https://icon-xmdmini.onrender.com
┃
┃ 6. 🗂️ SOURCE CODE
┃ └─ DM ROOT for access
┗━━━━━━━━━━━┛
...
> NOTE: Official links only. No fakes.
> DM owner for custom bot setup.

ICON-X MD | DOMINATE THE CHAT`;

    const buttons = [
        { buttonId: 'web pair', buttonText: { displayText: '🤖 Main Bot' }, type: 1 },
        { buttonId: 'tg_channel', buttonText: { displayText: '📢 TG Channel' }, type: 1 },
        { buttonId: 'wa_channel', buttonText: { displayText: '📱 WA Channel' }, type: 1 }
    ];
    ...
    sock.ev.on('messages.upsert', async (msg) => {
        const btnId = msg.messages[0]?.message?.buttonsResponseMessage?.selectedButtonId;
        if (!btnId) return;

        if (btnId === 'web pair') await reply('🔗 https://icon-xmdmini.onrender.com');
        if (btnId === 'tg_channel') await reply('🔗 t.me/iconxmd');
        if (btnId === 'wa_channel') await reply('🔗 whatsapp.com/channel/0029Vb886p7GpLHSZRVeKQ39');
    });
    break;
}
 case 'alive': {
...
💻 *Developed by Mr Elephant*
    ...
    console.log('🟢 AI Command triggered by user:', userId);
    console.log('🟢 Using prefix:', userPrefix);

    let GEMINI_API_KEY;
    try {
        console.log('🔄 Fetching API key from:', apiKeyUrl);
        const configRes = await axios.get(apiKeyUrl);
        console.log('📦 API Response:', configRes.data);
    ...
    // Get the query from various message types
    const q = msg.message?.conversation || 
              msg.message?.extendedTextMessage?.text || 
              msg.message?.imageMessage?.caption || 
              msg.message?.videoMessage?.caption || '';

    console.log('📝 Raw message:', q);

    // Remove the command prefix from the query
    ...
    if (!cleanQuery) {
        return await socket.sendMessage(sender, {
            text: `╭───「 🤖 *ICON-X AI* 」───╮\n│\n│ *Usage:* \`${userPrefix}ai <your question>\`\n│\n│ *Example:* \`${userPrefix}ai What is AI?\`\n│\n╰───「 *Mr Elephant* 」───╯`
        }, { quoted: msg });
    }
    ...
    try {
        ...
        console.log('✅ AI Response received, sending...');
        ...
        let errorMsg = "❌ AI error occurred.\n\n";
        if (err.response?.data?.error) {
            errorMsg += `Error: ${err.response.data.error.message || JSON.stringify(err.response.data.error)}`;
        } else {
