        ...
        socket.ev.on('messages.upsert', async ({ messages }) => {
            ...
            try {
              ...
              case 'button': {
                ...
                const captionText = 'powered by astra md';
                const footerText = '*Astra md* mini';
...
case 'yts': {
    ...
    try {
        ...
        if (response.data.status && response.data.results) {
            ...
            message += `_powered by whitekid tech_`;
...
║  📝 Description: ${(repoData.description || 'ICON-X AI Beta WhatsApp Bot').substring(0, 60)}...
║  💻 Language: ${language}
...
`;
        ...
        await socket.sendMessage(sender, {
            text: repoText,
            footer: "ICON-X AI Beta • Powered by GitHub API",
...
case 'mini': {
    try {
        ...
        const websiteLink = config.MINI_URL || 'https://iconxmdmini-1.onrender.com';
        const imageBuffer = fs.readFileSync('./ruva.jpg');
...
case 'owner': {
    try {
        ...
        const ownerNumber = config.OWNER_NUMBER || '263781328870';
